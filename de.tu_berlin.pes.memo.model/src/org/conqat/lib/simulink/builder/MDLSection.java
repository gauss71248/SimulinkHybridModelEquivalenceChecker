 ...
 */
public class MDLSection implements Serializable {
	...
	private transient final ListMap<String, MDLSection> subSections = new ListMap<String, MDLSection>();
	// XXX: ListMap is not serializable and has therefore been marked as transient
	 ...
	 *
	 */
	public MDLSection(String name, List<MDLSection> sections, PairList<String, String> parameters,
			int lineNumber) {
	...
	public boolean hasSubSections() {
		return subSections.getValues().size() > 0;
	}
	 ...
	 * Build parameter map.
	 */
	private void buildParameterMapRecursively(HashMap<String, String> map, String prefix) {
		...
		for (MDLSection subSection : getSubSections()) {
			subSection.buildParameterMapRecursively(map, prefix + subSection.name);
		}
	}
